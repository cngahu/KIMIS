                ...
                <div class="p-4 text-center shadow-sm border rounded-3" style="background:#fff; border-color:#e5e7eb;">
                    ...
                    @if($result)
                        <div class="mt-4">
                            ...
                            <div class="alert alert-success mb-0" style="font-size:14px; text-align:left;">
                                @php
                                    $name  = $result->studentsname ?? 'The trainee';
                                    $course = $result->coursename ?? 'the specified course';
                                @endphp

                                <strong>{{ $name }}</strong> is trained on
                                <strong>{{ $course }}</strong>
                                and thus the certificate is valid.

                                @if(!empty($result->certno))
                                    <div class="mt-1 small text-muted">
                                        Certificate No: {{ $result->certno }}
                                    </div>
                                @endif
                            </div>
